...
if not isinstance(pv_train, str) or not train_re.match(pv_train):
    fail(f"'truenas_preview.train' missing or malformed (got {pv_train!r}); expected capitalized word (e.g. Halfmoon)")
