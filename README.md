...
> **Migration note:** the one line installer below runs the `install.sh` attached to the current **Latest** release, and only kernel-tagged releases (`k<kernel>-gasket...`) ship the kernel-matching installer. Until the first of those is promoted to Latest, the one liner still matches your exact TrueNAS version, so only versions with their own promoted `v<version>-...` release install that way; the daily check keeps building every new TrueNAS version until then.
