...
The one line installer currently matches your exact TrueNAS version only. For another version on the same row, download that release's `coral.raw` and pass the file path to `install.sh` directly. This limitation goes away when kernel matched installs land (phase 2 of the migration).
