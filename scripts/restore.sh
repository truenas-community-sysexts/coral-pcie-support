#!/usr/bin/env bash
# Restores the original state by removing coral.raw sysext.
# Run this to completely remove the Coral TPU driver extension.
...
for arg in "$@"; do
    case "$arg" in
        ...
        --help)
            ...
            echo ""
            echo "Options:"
            echo "  --force    Proceed even if apex/gasket are in use (requires reboot afterward)"
...
# callers that don't fetch coral-lib.sh alongside restore.sh.
_source_coral_lib() {
    local dir
...
echo "=== Removing Coral TPU sysext ==="

# Pre-check: refuse to proceed if apex is in use unless --force is given.
# A held module means the backing .ko will be pulled out from under running
# consumers, leaving a half-state that confuses on next operation.
# Unload order: apex first (depends on gasket), then gasket.
NEEDS_REBOOT=0
for mod in apex gasket; do
    if lsmod 2>/dev/null | awk '{print $1}' | grep -qx "$mod"; then
        ...
        if [ "${REFCNT:-0}" -gt 0 ]; then
            ...
            # Show which processes hold the device open (only for apex)
            if [ "$mod" = "apex" ] && command -v fuser >/dev/null 2>&1; then
                PIDS=$(fuser /dev/apex* 2>/dev/null | tr -s ' ') || true
                if [ -n "$PIDS" ]; then
                    echo "  PIDs using /dev/apex*: ${PIDS}"
                    for pid in $PIDS; do
...
# Remove the coral sysext symlink and unmerge to drop the /usr overlay. Plain
...
# everything first and re-merge the survivors below. The coral.raw image itself
# lives on the data pool and is removed with the persistent config further down.
echo "Removing coral sysext..."
...
INIT_LOOKUP=$(coral_init_script_lookup)
...
if [ "$NEEDS_REBOOT" = "1" ]; then
    echo ""
    echo "============================================================"
    echo "  WARNING: apex/gasket modules were still in use when removed."
    echo "  The modules remain loaded in memory but their backing files"
    echo "  are gone. A REBOOT IS REQUIRED to fully clean up."
    echo "  Stop any Coral consumers (e.g. Frigate) before rebooting."
    echo "============================================================"
fi
